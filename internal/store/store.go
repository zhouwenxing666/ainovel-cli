...
func (s *Store) FoundationMissing() ([]string, error) {
	...
	// 新书必须在 foundation_audit 前生成完整封面章节；旧 writing 项目若完全没有
	// 该二级标题则触发一次自动迁移，但已有手工章节不覆盖。Complete 是终态，不迁移。
	if planning && premise != "" {
	...
	} else if progress != nil && progress.Phase == domain.PhaseWriting && !domain.HasCoverPromptSection(premise) {
		missing = append(missing, "cover_prompt")
	}
	// 新书只有经过模型对已落盘工件的显式语义审查，才允许从规划进入写作。
	// PhaseWriting/Complete 代表旧书或已审查的新书，不再要求补做审查；writing
	// 旧书的封面迁移已在上方单独处理。审查是动作而非文件缺失，因此只在其它
	// 规划工件齐全时追加。
	if len(missing) == 0 {
		if planning {
